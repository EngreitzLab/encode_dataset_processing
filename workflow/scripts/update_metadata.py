...
@click.option("--new_metadata")
def main(orig_metadata, metadata_synapse, new_metadata):
    ...
    merged_df = orig_metadata_df.merge(metadata_syn_df, left_on=left_col, right_on=right_col, how='left')
    columns = orig_metadata_df.columns.to_list() + get_synapse_columns(metadata_syn_df.columns.to_list())
    assert len(merged_df) == len(orig_metadata_df), "Cannot properly merge metadata 1:1"
    merged_df.fillna("NA", inplace=True)
    merged_df[columns].to_csv(new_metadata, sep='\t', index=False)
